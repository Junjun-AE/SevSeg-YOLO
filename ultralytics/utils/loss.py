...
# =============================================================================
# SevSeg-YOLO: Score Loss Components (M3)
# =============================================================================


class ScoreLoss:
    """Score loss with configurable loss function.

    Supports two modes:
        - 'gaussian_nll': 0.5 * (pred - gt)^2 / sigma^2  (default, best performer)
        - 'smooth_l1': SmoothL1Loss with configurable beta

    Computes loss only for foreground (assigned) anchors with non-NaN GT scores.

    Args:
        gaussian_sigma: Standard deviation for Gaussian NLL (on [0,1] scale). Default 0.1.
        loss_type: 'gaussian_nll' or 'smooth_l1'. Default 'gaussian_nll'.
        smooth_l1_beta: Beta parameter for Smooth L1 loss. Default 0.2.
    """

    def __init__(self, gaussian_sigma=0.1, loss_type='gaussian_nll', smooth_l1_beta=0.2):
        self.gaussian_sigma = gaussian_sigma
        self.loss_type = loss_type
        self.smooth_l1_beta = smooth_l1_beta

    def __call__(self, pred_flat, gt_scores_padded, fg_mask, target_gt_idx, batch):
        """Compute score loss for assigned positive anchors.

        Args:
            pred_flat: (B, N) Score Head predictions in [0, 1].
            gt_scores_padded: (B, max_gt, 1) GT scores in [0, 10], NaN for unlabeled.
            fg_mask: (B, N) bool, foreground mask from assigner.
            target_gt_idx: (B, N) int, GT index per anchor.
            batch: Original batch dict (unused, kept for interface compatibility).
        ...
        # Get all foreground indices at once (vectorized)
        fg_indices = fg_mask.nonzero(as_tuple=False)  # (N_pos, 2) = [batch_idx, anchor_idx]

        if fg_indices.numel() == 0:
            return pred_flat.sum() * 0  # DDP safe

        bi = fg_indices[:, 0]  # batch indices (N_pos,)
        ai = fg_indices[:, 1]  # anchor indices (N_pos,)

        # Batch gather — single indexing, no for loop
        pred_pos = pred_flat[bi, ai]  # (N_pos,)
        gt_idx = target_gt_idx[bi, ai]  # (N_pos,)
        gt_raw = gt_scores_padded[bi, gt_idx, 0]  # (N_pos,)

        # Filter NaN (instances without score annotations)
        valid = ~torch.isnan(gt_raw)
        if not valid.any():
            return pred_flat.sum() * 0  # DDP safe

        pred_valid = pred_pos[valid]
        gt_norm = gt_raw[valid] / 10.0  # normalize to [0, 1]

        if self.loss_type == 'smooth_l1':
            # Smooth L1: |x| < beta → 0.5*x^2/beta, else |x| - 0.5*beta
            regression_loss = F.smooth_l1_loss(pred_valid, gt_norm, beta=self.smooth_l1_beta, reduction='mean')
        else:
            # Gaussian NLL: loss = 0.5 * (pred - gt)^2 / sigma^2 (default)
            sigma = max(self.gaussian_sigma, 1e-6)
            regression_loss = (0.5 * ((pred_valid - gt_norm) ** 2) / (sigma ** 2)).mean()

        return regression_loss


class v8ScoreDetectionLoss(v8DetectionLoss):
    """Detection + Score joint loss, following v8SegmentationLoss pattern."""

    def __init__(self, model, tal_topk=10, tal_topk2=None):
        super().__init__(model, tal_topk, tal_topk2)
        score_cfg = getattr(model, 'args', None)
        score_cfg = getattr(score_cfg, 'score', {}) if score_cfg else {}
        if isinstance(score_cfg, dict):
            self.score_loss_fn = ScoreLoss(
                gaussian_sigma=score_cfg.get('gaussian_sigma', 0.1),
                loss_type=score_cfg.get('loss_type', 'gaussian_nll'),
                smooth_l1_beta=score_cfg.get('smooth_l1_beta', 0.2),
            )
            self.lambda_score = score_cfg.get('lambda_score', 0.05)
        else:
            self.score_loss_fn = ScoreLoss()
            self.lambda_score = 0.05

    def __call__(self, preds, batch):
        """Override to return 2-tuple for BaseTrainer compatibility.

        BUG-NEW-1 fix: loss() returns 3-tuple (total, items, score_grad) so that
        ScoreE2ELossV3 can access score_grad for alpha weighting. But BaseTrainer
        unpacks as: loss, self.loss_items = model.loss(batch, preds)
        which requires exactly 2 values. This override ensures non-E2E mode works.
        ScoreE2ELossV3 bypasses __call__ and invokes .loss() directly.
        """
        result = self.loss(self.parse_output(preds), batch)
        return result[0], result[1]  # (loss_sum, loss_items_detached)

    def loss(self, preds, batch):
        """Detection + Score joint loss returning 4-element loss tensor.

        Returns:
            3-tuple: (loss_sum * bs, loss_items_detached, score_loss_with_grad).
        """
        (fg_mask, target_gt_idx, target_bboxes, _, _), det_loss, det_loss_detach = \
            self.get_assigned_targets_and_loss(preds, batch)

        bs = preds['boxes'].shape[0]
        IDX_SCORE = 3

        # 4-element loss: [box, cls, dfl(=0 for YOLO26), score]
        ...
        # Score Loss
        score_preds_flat = preds.get('score_preds', None)
        if score_preds_flat is not None and fg_mask.sum() > 0:
            gt_scores_padded = self._preprocess_scores(batch, bs)
            if gt_scores_padded is not None:
                pred_flat = score_preds_flat.squeeze(1)  # (B, 1, N) → (B, N)
                score_loss = self.score_loss_fn(
                    pred_flat, gt_scores_padded, fg_mask, target_gt_idx, batch
                )
                loss[IDX_SCORE] = self.lambda_score * score_loss

        # DDP defense: ensure score_head parameters stay in computation graph
        # Only add zero-sum if score_loss has no gradient (e.g., no valid samples)
        if score_preds_flat is not None and not loss[IDX_SCORE].requires_grad:
            loss[IDX_SCORE] = loss[IDX_SCORE] + (score_preds_flat * 0).sum()

        # Return 3-tuple: (total_loss_with_grad, loss_items_detached, score_loss_with_grad)
        # The 3rd element allows ScoreE2ELossV3 to apply alpha weights WITH gradient flow.
        score_loss_grad = loss[IDX_SCORE] * bs  # score component with gradient, scaled by bs
        return loss.sum() * bs, loss.detach(), score_loss_grad

    def _preprocess_scores(self, batch, batch_size):
        """Reshape batch['scores'] from flat (total_N, 1) to padded (B, max_gt, 1).

        NaN-filled for missing scores. Handles batches where some images have
        no annotations (batch_idx may not contain all indices 0..batch_size-1).
        """
        scores = batch.get('scores', None)
        if scores is None:
            return None
        scores = scores.to(self.device).view(-1, 1)
        batch_idx = batch['batch_idx'].to(self.device).long()

        if batch_idx.numel() == 0:
            return None

        unique_imgs, counts = batch_idx.unique(return_counts=True)
        max_gt = counts.max().int().item()
        gt_scores = torch.full((batch_size, max_gt, 1), float('nan'), device=self.device)

        # Build within-image index for each GT annotation
        # e.g. batch_idx=[0,0,0,2,2] → within_idx=[0,1,2,0,1]
        # (image 1 has no annotations, image 0 has 3, image 2 has 2)
        within_idx = torch.zeros_like(batch_idx)
        for i, (img_i, cnt) in enumerate(zip(unique_imgs, counts)):
            mask = batch_idx == img_i
            within_idx[mask] = torch.arange(cnt, device=self.device)

        gt_scores[batch_idx, within_idx, :] = scores
        return gt_scores


class ScoreE2ELossV3(E2ELoss):
    """E2E loss with score-aware loss separation.

    Overrides __call__ to:
    1. Use fixed weights for score_loss (not decayed by o2m/o2o)
    2. Merge loss_items from both branches for complete training monitoring
    """

    def __init__(self, model):
        super().__init__(model, loss_fn=v8ScoreDetectionLoss)
        # GAP-6/A18: Read alpha from config, default 0.5/0.5
        score_cfg = getattr(model, 'args', None)
        score_cfg = getattr(score_cfg, 'score', {}) if score_cfg else {}
        if not isinstance(score_cfg, dict):
            score_cfg = {}
        self.alpha_score_o2m = score_cfg.get('alpha_score_o2m', 0.5)
        self.alpha_score_o2o = score_cfg.get('alpha_score_o2o', 0.5)

    def __call__(self, preds, batch):
        """Compute E2E loss with separated det/score weighting.

        det_loss follows o2m/o2o decay (original behavior).
        score_loss uses fixed alpha weights WITH gradient flow (BUG-CRITICAL-1 fix).

        v8ScoreDetectionLoss.loss() now returns 3-tuple:
            (total_loss_with_grad, loss_items_detached, score_loss_with_grad)
        """
        preds = self.one2many.parse_output(preds)
        one2many, one2one = preds["one2many"], preds["one2one"]
        loss_one2many = self.one2many.loss(one2many, batch)  # 3-tuple
        loss_one2one = self.one2one.loss(one2one, batch)     # 3-tuple

        IDX_SCORE = 3

        # Extract score loss WITH gradient from 3rd tuple element
        score_grad_o2m = loss_one2many[2]  # score_loss * bs, HAS gradient
        score_grad_o2o = loss_one2one[2]   # score_loss * bs, HAS gradient

        # det = total - score (both have gradient, subtraction preserves grad)
        det_grad_o2m = loss_one2many[0] - score_grad_o2m
        det_grad_o2o = loss_one2one[0] - score_grad_o2o

        # det: decayed by o2m/o2o (original behavior)
        # score: fixed alpha weights (gradient flows correctly through alpha * score_grad)
        loss_sum = (det_grad_o2m * self.o2m + det_grad_o2o * self.o2o +
                    score_grad_o2m * self.alpha_score_o2m +
                    score_grad_o2o * self.alpha_score_o2o)

        # loss_items: merge both branches for training log (detached, display only)
        # SAFETY: loss_one2many[1] and loss_one2one[1] are already detached (from loss.detach()),
        # but we explicitly detach+clone as defensive measure against future changes in
        # v8ScoreDetectionLoss.loss() return semantics. See Issue #7/#18.
        loss_items = loss_one2many[1].detach().clone()
        loss_items[:IDX_SCORE] = (loss_one2many[1][:IDX_SCORE] * self.o2m +
                                   loss_one2one[1][:IDX_SCORE] * self.o2o)
        loss_items[IDX_SCORE] = (loss_one2many[1][IDX_SCORE] * self.alpha_score_o2m +
                                  loss_one2one[1][IDX_SCORE] * self.alpha_score_o2o)
