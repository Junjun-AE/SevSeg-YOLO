...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        cls = []
        instances = []
        scores_list = []  # SevSeg-YOLO
        imgsz = self.imgsz * 2  # mosaic imgsz
        for labels in mosaic_labels:
            cls.append(labels["cls"])
            instances.append(labels["instances"])
            # SevSeg-YOLO: collect scores (NaN fallback for labels without scores)
            scores_list.append(labels.get("scores", np.full((len(labels["cls"]), 1), np.nan, dtype=np.float32)))
        # Final labels
        final_labels = {
            ...
            "scores": np.concatenate(scores_list, 0),  # SevSeg-YOLO
        }
        final_labels["instances"].clip(imgsz, imgsz)
        good = final_labels["instances"].remove_zero_area_boxes()
        final_labels["cls"] = final_labels["cls"][good]
        final_labels["scores"] = final_labels["scores"][good]  # SevSeg-YOLO: sync filter
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        # SevSeg-YOLO: concat scores for MixUp
        if "scores" in labels and "scores" in labels2:
...
class CutMix(BaseMixTransform):
    ...
    def _mix_transform(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        # SevSeg-YOLO: concat scores for CutMix
        if "scores" in labels and "scores" in labels2:
...
class RandomPerspective:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        labels["cls"] = cls[i]
        # SevSeg-YOLO: sync scores with box filtering
        if "scores" in labels:
            labels["scores"] = labels["scores"][i]
        labels["img"] = img
...
class CopyPaste(BaseMixTransform):
    ...
    def _transform(self, labels1: dict[str, Any], labels2: dict[str, Any] = {}) -> dict[str, Any]:
        ...
        cls = labels1["cls"]
        # SevSeg-YOLO: track scores through CopyPaste
        cp_scores = labels1.get("scores", np.full((len(cls), 1), np.nan, dtype=np.float32))
        h, w = im.shape[:2]
        ...
        # SevSeg-YOLO: get source scores for copy targets
        src_scores = labels2.get("scores", cp_scores)
        for j in indexes[: round(self.p * n)]:
            cls = np.concatenate((cls, labels2.get("cls", cls)[[j]]), axis=0)
            instances = Instances.concatenate((instances, instances2[[j]]), axis=0)
            # SevSeg-YOLO: append score for copied instance
            cp_scores = np.concatenate((cp_scores, src_scores[[j]]), axis=0)
        ...
        labels1["scores"] = cp_scores  # SevSeg-YOLO
        return labels1
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.contains_spatial:
            ...
            if len(cls):
                ...
                # SevSeg-YOLO: save original bboxes for score index recovery
                orig_bboxes = bboxes.copy() if "scores" in labels else None
                ...
                if len(new["class_labels"]) > 0:  # skip update if no bbox in new im
                    ...
                    # SevSeg-YOLO: recover score indices by matching surviving bboxes
                    # FIX #20: Use unique matching to avoid duplicate index when bboxes overlap
                    if "scores" in labels and orig_bboxes is not None and len(bboxes) != len(orig_bboxes):
                        keep_idx = []
                        used = set()
                        for nb in bboxes:
                            dists = np.abs(orig_bboxes - nb).sum(axis=1)
                            # Find closest unused original bbox to avoid duplicate matches
                            ...
                            for si in sorted_idx:
                                ...
                                keep_idx.append(np.argmin(dists))  # fallback
                        labels["scores"] = labels["scores"][np.array(keep_idx)]
...
class Format:
    ...
    def __call__(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        # SevSeg-YOLO: convert scores to tensor
        if "scores" in labels:
            s = labels["scores"]
            labels["scores"] = torch.from_numpy(s).float() if nl and s is not None else torch.full((nl, 1), float('nan'))
        return labels
