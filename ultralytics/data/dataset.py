...
DATASET_CACHE_VERSION = "1.2.0"  # SevSeg-YOLO v0.4.5: mixed 5/6 column fix
...
class YOLODataset(BaseDataset):
    ...
    def cache_labels(self, path: Path = Path("./labels.cache")) -> dict:
        ...
        with ThreadPool(NUM_THREADS) as pool:
            ...
            for im_file, lb, shape, segments, keypoint, nm_f, nf_f, ne_f, nc_f, msg, scores in pbar:
                nm += nm_f
                nf += nf_f
                ne += ne_f
                nc += nc_f
                    ...
                    x["labels"].append(
                        {
                            ...
                            "normalized": True,
                            "bbox_format": "xywh",
                            # SevSeg-YOLO: inject scores (NaN if not available)
                            "scores": scores if scores is not None else np.full(
                                (lb.shape[0], 1), np.nan, dtype=np.float32
                            ),
                        }
                    )
                if msg:
    ...
    def get_labels(self) -> list[dict]:
        ...
        # SevSeg-YOLO: ensure all labels have scores field (NaN fallback for old caches)
        for lb in labels:
            if "scores" not in lb:
                n = len(lb["cls"])
                lb["scores"] = np.full((n, 1), np.nan, dtype=np.float32)
    ...
    def collate_fn(batch: list[dict]) -> dict:
        ...
        for i, k in enumerate(keys):
            ...
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb", "scores"}:
