...
def validate_label_line(parts, num_cls=80):
    """Validate a single label line from a YOLO annotation file (§9.5 / A2).

    Args:
        parts: List of string tokens from one line of a label file.
        num_cls: Number of classes in the dataset.

    Returns:
        (is_valid, error_msg, has_score): Validation result tuple.
    """
    n = len(parts)
    has_score = False
    ...
    if n == 6:
        ...
        return True, "", False  # segment format
    else:
        return False, f"Expected 5 or 6 columns, got {n}", False

    try:
        vals = [float(x) for x in parts]
    except ValueError:
        return False, "Non-numeric value in label line", has_score

    cls_id = int(vals[0])
    if cls_id < 0 or cls_id >= num_cls:
        return False, f"Class {cls_id} out of range [0, {num_cls})", has_score

    for i, c in enumerate(vals[1:5]):
        if c < -0.01 or c > 1.01:
            return False, f"Coordinate[{i}]={c} out of [0,1] range", has_score

    if has_score and (vals[5] < -1.1 or vals[5] > 10.1):
        return False, f"Score {vals[5]} out of [-1,10] range", has_score
    # -1 is a valid NaN placeholder for missing scores (from convert_labelme.py)
    # verify_image_label will convert -1 → NaN during loading

    return True, "", has_score
...
def verify_image_label(args: tuple) -> list:
    ...
    scores = None  # SevSeg-YOLO: severity score column (6th column)
    try:
        # Verify images
        im = Image.open(im_file)
        im.verify()  # PIL verify
        ...
        if os.path.isfile(lb_file):
            ...
            with open(lb_file, encoding="utf-8") as f:
                ...
                if any(len(x) > 6 for x in lb) and (not keypoint):  # is segment
                    ...
                    # SevSeg-YOLO: normalize mixed 5/6 columns BEFORE np.array()
                    # If some lines have 5 cols and others 6, pad 5-col lines to 6 with NaN
                    col_counts = set(len(x) for x in lb if len(x) in (5, 6))
                    if col_counts == {5, 6}:
                        lb = [x + ["nan"] if len(x) == 5 else x for x in lb]
            ...
            if nl := len(lb):
                if keypoint:
                    ...
                    # SevSeg-YOLO: support 6-column labels (cls, x, y, w, h, score)
                    scores = None
                    if lb.shape[1] == 6:
                        scores = lb[:, 5:6].copy()  # (n, 1), preserve original values [0, 10]
                        # Convert -1 placeholder (from convert_labelme.py) to NaN
                        scores[scores < -0.5] = np.nan
                        # Validate score range with tolerance (NaN passes comparison)
                        valid_scores = scores[~np.isnan(scores)]
                        if len(valid_scores) > 0:
                            assert (valid_scores >= -0.1).all() and (valid_scores <= 10.1).all(), (
                                f"score values out of range [0, 10]: min={valid_scores.min():.2f}, max={valid_scores.max():.2f}"
                            )
                        scores = np.where(np.isnan(scores), np.nan, np.clip(scores, 0.0, 10.0))
                        lb = lb[:, :5]  # truncate to standard 5 columns
                    elif lb.shape[1] == 5:
                        pass  # standard YOLO format, no scores
                    else:
                        assert False, f"labels require 5 or 6 columns, {lb.shape[1]} columns detected"
                    points = lb[:, 1:]
                ...
                if len(i) < nl:  # duplicate row check
                    ...
                    # SevSeg-YOLO: sync scores with dedup
        ...
        lb = lb[:, :5]
        # SevSeg-YOLO: return scores as 11th element (None if no scores)
        return im_file, lb, shape, segments, keypoints, nm, nf, ne, nc, msg, scores
    except Exception as e:
        nc = 1
        msg = f"{prefix}{im_file}: ignoring corrupt image/label: {e}"
...
class HUBDatasetStats:
    ...
    def get_json(self, save: bool = False, verbose: bool = False) -> dict:
        ...
        def _round(labels):
            ...
            if self.task in {"detect", "score_detect"}:
