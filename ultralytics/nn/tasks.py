...
class ScoreDetectionModel(DetectionModel):
    """SevSeg-YOLO detection model with severity score prediction.

    Uses ScoreDetect head (registered in yaml as last layer).
    Only overrides init_criterion to use ScoreE2ELossV3 / v8ScoreDetectionLoss.
    The score_head is already constructed inside ScoreDetect.__init__ by parse_model.
    """

    def init_criterion(self):
        """Replace loss function with score-aware version."""
        # Import here to avoid circular imports (loss.py imports from tasks.py)
        # These classes will be implemented in M3; for now fall back to base E2ELoss
        try:
            from ultralytics.utils.loss import ScoreE2ELossV3, v8ScoreDetectionLoss
            return ScoreE2ELossV3(self) if getattr(self, "end2end", False) else v8ScoreDetectionLoss(self)
        except ImportError:
            # M3 not yet implemented — fall back to base loss for M2 testing
            return E2ELoss(self) if getattr(self, "end2end", False) else v8DetectionLoss(self)
...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            if m in {Detect, YOLOEDetect, Segment, Segment26, YOLOESegment, YOLOESegment26, Pose, Pose26, OBB, OBB26, ScoreDetect}:
                m.legacy = legacy
            # ITEM-3: Pass score_mid_ratio from YAML to ScoreDetect (§22.4.2 ablation B1-B3)
            if m is ScoreDetect:
                score_mid_ratio = d.get("score_mid_ratio", 0.25)
                args.append(score_mid_ratio)
        elif m is v10Detect:
