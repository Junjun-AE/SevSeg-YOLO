...
# =============================================================================
# SevSeg-YOLO: ScoreHead + ScoreDetect
# =============================================================================


class ScoreHead(nn.Module):
    """SevSeg-YOLO severity score prediction head.

    Predicts a normalized [0, 1] severity score for each detection position.
    Three scales (P3/P4/P5) each have independent parameter branches.
    Architecture aligned with YOLO26 DWConv cls head (cv3) style.

    Args:
        ch (tuple): Input channel counts per scale (ch_p3, ch_p4, ch_p5).
        mid_ratio (float): Mid-channel ratio. mid_ch = max(int(ch_i * mid_ratio), 8).
    """

    def __init__(self, ch: tuple, mid_ratio: float = 0.25):
        super().__init__()
        self.nl = len(ch)
        self.heads = nn.ModuleList()
        for c_in in ch:
            c_mid = max(int(c_in * mid_ratio), 8)
            self.heads.append(
                nn.Sequential(
                    DWConv(c_in, c_in, 3),    # depthwise separable conv
                    Conv(c_in, c_mid, 1),      # 1x1 reduce
                    DWConv(c_mid, c_mid, 3),   # depthwise separable conv
                    Conv(c_mid, c_mid, 1),     # 1x1
                    nn.Conv2d(c_mid, 1, 1),    # final output 1 channel
                )
            )
        self._init_weights()

    def _init_weights(self):
        """Initialize last Conv2d with small weights for near-zero initial output."""
        for head in self.heads:
            last_conv = head[-1]  # nn.Conv2d(c_mid, 1, 1)
            nn.init.normal_(last_conv.weight, std=0.001)
            if last_conv.bias is not None:
                nn.init.constant_(last_conv.bias, 0.0)

    def forward(self, features: list[torch.Tensor]) -> list[torch.Tensor]:
        """Forward pass: produce clamped [0, 1] score maps per scale.

        Args:
            features: [P3, P4, P5] feature maps.

        Returns:
            List of score maps, each (B, 1, H_i, W_i) in [0, 1].
        """
        return [torch.clamp(head(feat), 0.0, 1.0) for feat, head in zip(features, self.heads)]

    def forward_scale(self, x: list[torch.Tensor], bs: int) -> torch.Tensor:
        """Return flattened + concatenated score tensor (B, 1, N).

        Args:
            x: [P3, P4, P5] feature maps.
            bs: Batch size.

        Returns:
            (B, 1, N) where N = sum(H_i * W_i), values clamped to [0, 1].
        """
        return torch.cat(
            [torch.clamp(self.heads[i](x[i]), 0.0, 1.0).view(bs, 1, -1) for i in range(self.nl)],
            dim=2,
        )

    # Backward compatibility alias (§3.6)
    forward_flat = forward_scale


class ScoreDetect(Detect):
    """Detection head with severity score prediction for SevSeg-YOLO.

    Extends Detect with a ScoreHead branch, following the exact same pattern as
    Segment(Detect): forward_head injection → _inference readout → postprocess gather.

    Attributes:
        ns (int): Number of score channels (always 1).
        score_head (ScoreHead): One-to-many score prediction head.
        one2one_score_head (ScoreHead): One-to-one score prediction head (deepcopy).
    """

    ns = 1  # severity score channels (class attribute)

    def __init__(self, nc: int = 80, reg_max=1, end2end=True, ch: tuple = (), score_mid_ratio: float = 0.25):
        """Initialize ScoreDetect.

        Args:
            nc: Number of classes.
            reg_max: DFL channels (1 for YOLO26).
            end2end: End-to-end NMS-free mode.
            ch: Channel sizes per scale from backbone.
            score_mid_ratio: Mid-channel ratio for ScoreHead.
        """
        super().__init__(nc, reg_max, end2end, ch)
        self.score_head = ScoreHead(ch, score_mid_ratio)
        if end2end:
            self.one2one_score_head = copy.deepcopy(self.score_head)

    @property
    def one2many(self):
        """One-to-many heads including score_head."""
        return dict(box_head=self.cv2, cls_head=self.cv3, score_head=self.score_head)

    @property
    def one2one(self):
        """One-to-one heads including one2one_score_head."""
        return dict(box_head=self.one2one_cv2, cls_head=self.one2one_cv3, score_head=self.one2one_score_head)
    ...
    def forward_head(
        ...
        """Inject score_preds into the prediction dict.

        Follows the Segment.forward_head pattern exactly:
            Segment: preds["mask_coefficient"] = cat([mask_head[i](x[i])...])
            ScoreDetect: preds["score_preds"] = score_head.forward_scale(x, bs)
        """
        preds = super().forward_head(x, box_head, cls_head)
        if score_head is not None:
            bs = x[0].shape[0]
            preds["score_preds"] = score_head.forward_scale(x, bs)  # (B, 1, N)
        return preds

    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        """Append score_preds to detection output during inference.

        Follows Segment._inference pattern:
            Segment: cat([det, x["mask_coefficient"]])
            ScoreDetect: cat([det, x["score_preds"]])
        """
        det = super()._inference(x)  # (B, 4+nc, N)
        if "score_preds" in x:
            return torch.cat([det, x["score_preds"]], dim=1)  # (B, 4+nc+1, N)
        return det

    def postprocess(self, preds: torch.Tensor) -> torch.Tensor:
        """Post-process: split 4+nc+ns columns, gather by topk index.

        Follows Segment.postprocess pattern:
            Segment: split([4, nc, nm]) → gather mask_coeff
            ScoreDetect: split([4, nc, ns]) → gather severity

        Returns:
            (B, K, 7): [x1, y1, x2, y2, max_cls_prob, cls_index, severity]
        """
        boxes, scores, severity = preds.split([4, self.nc, self.ns], dim=-1)
        scores, conf, idx = self.get_topk_index(scores, self.max_det)
        boxes = boxes.gather(dim=1, index=idx.repeat(1, 1, 4))
        severity = severity.gather(dim=1, index=idx.repeat(1, 1, self.ns))
        return torch.cat([boxes, scores, conf, severity], dim=-1)

    def fuse(self) -> None:
        """Remove one2many heads for inference optimization."""
        super().fuse()  # cv2 = cv3 = None
